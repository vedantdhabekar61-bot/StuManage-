...
interface WhatsAppReminderButtonProps { student: Student; className?: string; showText?: boolean; }
...
export function WhatsAppReminderButton({ student, className, showText = true }: WhatsAppReminderButtonProps) {
  ...
  const handleSend = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isOpening) return;
    setIsOpening(true); setShowToast(true); setError(null);
    ...
    if (!success) setError("WhatsApp could not be opened. Please check if popups are allowed.");
  ...
  return (
    <>
      <button onClick={handleSend} disabled={isOpening} className={cn("transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed", className)}>
        {isOpening ? <Loader2 className="h-5 w-5 animate-spin" /> : <MessageCircle className="h-5 w-5 fill-current" />}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-foreground/40 backdrop-blur-sm p-6">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="flex flex-col items-center gap-6 text-center">
                <div className="rounded-[1.5rem] bg-rose-50 p-5 text-rose-600"><MessageCircle className="h-10 w-10" /></div>
                <div><h3 className="text-xl font-bold text-foreground">WhatsApp Error</h3><p className="text-sm font-medium text-muted mt-2 leading-relaxed">WhatsApp not installed or popup blocked.</p></div>
                <button onClick={() => setError(null)} className="mt-2 w-full rounded-2xl bg-primary py-4 text-sm font-bold uppercase tracking-widest text-white shadow-lg shadow-primary/20 transition-all active:scale-95">Got it</button>
              </div>
            </motion.div>
          </div>
        )}
