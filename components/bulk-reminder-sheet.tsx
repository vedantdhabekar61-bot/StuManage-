...
import { X, MessageCircle, Users, CheckCircle2, Bell } from 'lucide-react';
...
export function BulkReminderSheet({ students, isOpen, onClose }: BulkReminderSheetProps) {
  const dueStudents = useMemo(() => {
    return students.filter(s => s.paymentStatus !== 'Paid').sort((a, b) => {
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="flex flex-col gap-4">
                {dueStudents.map((student) => (
                  <div
                    key={student.id}
                    className="flex items-center justify-between rounded-2xl border border-slate-50 bg-slate-50/50 p-4"
                  >
            ...
            <div className="border-t border-slate-50 p-6">
              <button
                onClick={onClose}
                className="w-full rounded-2xl bg-slate-900 py-4 text-sm font-bold uppercase tracking-widest text-white shadow-lg transition-all active:scale-95"
              >
                Done
              </button>
            </div>
